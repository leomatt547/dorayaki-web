...
request(`../../db/search-all-db.php?search=${query}`, function () {});

var cookie_name = "stotal";
var val = retrieve_cookie(cookie_name);
if (val) {
  total = JSON.parse(val)[0].stotal;
  delete_cookie("stotal", "/");
} else {
...
function changePage(page) {
  var hasl_cari = document.getElementById("cari");
  hasl_cari.innerText = "Hasil Pencarian: ".concat(query);
  ...
  post(
    ...
    `../../db/search-db.php?search=${query}&page=${page - 1}`,
    function () {
