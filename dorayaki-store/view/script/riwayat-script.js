...
request("../../db/riwayat-all-db.php", function () {
  console.log(this.responseText);
});

var cookie_name = "rtotal";
var val = retrieve_cookie(cookie_name);
if (val) {
  total = JSON.parse(val)[0].rtotal;
  delete_cookie("rtotal", "/");
} else {
  ...
  request(
    ...
    function (json) {
      ...
      if (json.length > 0) {
        ...
        const th3_isi = document.createTextNode("Username");
        th3.appendChild(th3_isi);

        var th4 = document.createElement("th");
        const th4_isi = document.createTextNode("Waktu");
        ...
        for (var i = 0; i < json.length; i++) {
          ...
          const td3_isi = document.createTextNode(json[i].username);
          td3.appendChild(td3_isi);

          var td4 = document.createElement("td");
          const td4_isi = document.createTextNode(json[i].waktu);
